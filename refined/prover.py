...
def get_claim_sources(query, n_websites, use_mojk=False):
    results_final = []
    if use_mojk:
        results = mojk_client.search(query, count=min(10, n_websites))
        for x in results:
...
def get_webdata_chunks(query,  n_websites):
    results = get_claim_sources(query, n_websites, use_mojk=False)
...
class Prover():
    """
    A class for generating arguments for and against a given proposition claim.

    Attributes:
        proposition_claim (str): The claim that the Prover will generate arguments for.
        opposition_claim (str, optional): The claim that the Prover will generate arguments against. If not provided, the opposition claim will be automatically generated by reversing the proposition claim.
        n_argument_clusters (int): The number of argument clusters to generate for each claim.
        n_chunks_needed_per_cluster (int): The minimum number of relevant chunks needed per argument cluster. This number must be even.
        use_small_model (bool): Whether to use a smaller model for generating the final judgment.
        n_websites (int): The number of websites to search for relevant data.

    Methods:
        run(): Generates arguments for and against the proposition claim, and returns the final judgment.
    """
    def __init__(self, proposition_claim, opposition_claim=None,n_argument_clusters = 3,n_chunks_needed_per_cluster = 10, use_small_model=True, n_websites=10):
