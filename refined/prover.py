...
def get_claim_sources(query, use_mojk=False, n_websites=10):
    results_final = []
    if use_mojk:
        results = mojk_client.search(
            query, count=min(10, n_websites)
        )  # 10 is the limit for current plan
        for x in results:
...
def save_source_pages(results):
    ...
    download_webpage_html(
        urls_for_download, filenames_for_download, save_folder=filedir
    )
    ...
    for sourced_chunk in tqdm(sourced_chunks, desc="Embedding chunks..."):
        if len(sourced_chunk) > 15:
            ...
            embedding = ollama.embeddings(
                model="nomic-embed-text", prompt=sourced_chunk
            )["embedding"]
...
def process_clusters(chunk_vector_pairs, n_argument_clusters):
    ...
    clustered = k_means(
        np.array([x[1] for x in chunk_vector_pairs]), n_argument_clusters
    )
    ...
    sampled_clusters = [
        x[0]
        for x in sorted(Counter(cluster_ids).items(), key=lambda x: x[1], reverse=True)
    ]
...
def get_n_informative_chunks(
    claim,
    cluster_to_chunk_dict,
    ...
    # Loop through clusters in the cluster_to_chunk_dict
    print(claim)
    for clust_i, chunks in tqdm(
        cluster_to_chunk_dict.items(),
        desc=f"Getting {n_chunks_needed_per_cluster} informative chunks per cluster",
    ):
        ...
        pbar = tqdm(
            range(n_chunks_needed_per_cluster),
            total=n_chunks_needed_per_cluster,
            desc=f"Cluster {clust_i}",
        )
        ...
        for _ in pbar:
            for chu in tqdm(sampled_chunks, desc="All Chunks..."):
                ...
                if informative.get("response", "").lower() == "true":
                    ...
                    pbar.update(
                        1
                    )  # Update the progress bar after finding an informative chunk
        ...
        if ct < n_chunks_needed_per_cluster:
            print(
                f"Warning: Only {ct} informative chunk(s) found for cluster {clust_i}."
            )
    ...
    while len(intermediate_summaries) != 1:
        ...
        for zx in range(0, len(intermediate_summaries), 2):
            if zx + 1 < len(intermediate_summaries):
                combined_claim = combine_claims(
                    claim, intermediate_summaries[zx], intermediate_summaries[zx + 1]
                )
...
def get_final_args(
    claim, cluster_to_chunk_dict, max_sampled_chunks_per_cluster, informative_chunks
):
...
def filter_chunks_using_vsim(query, all_chunk_vector_pairs, thresh=0.65):
    ...
    similarities = []
    for i, chunky in tqdm(
        enumerate(all_chunk_vector_pairs), total=len(all_chunk_vector_pairs)
    ):
    ...
    if len(results) < 10:
        results = filter_chunks_using_vsim(
            query, all_chunk_vector_pairs, thresh=max(thresh - 0.2, 0)
        )
    return results
...
def prover_F(
    ...
    # Embed chunks
    prop_all_chunk_vector_pairs = embed_chunks(prop_chunks)
    master_dict.update({"status": "Embedded proposition chunks", "progress": 50})
    yield master_dict

    opp_all_chunk_vector_pairs = embed_chunks(opp_chunks)
    master_dict.update({"status": "Embedded opposition chunks", "progress": 60})
    yield master_dict

    prop_reduced_chunk_vector_pairs = filter_chunks_using_vsim(
        opposition_query, prop_all_chunk_vector_pairs, 0.65
    )
    opp_reduced_chunk_vector_pairs = filter_chunks_using_vsim(
        opposition_query, opp_all_chunk_vector_pairs, 0.65
    )

    prop_cluster_dict = process_clusters(
        prop_reduced_chunk_vector_pairs, n_argument_clusters
    )
    ...
    opp_cluster_dict = process_clusters(
        opp_reduced_chunk_vector_pairs, n_argument_clusters
    )
    ...
    prop_informative_chunks = get_n_informative_chunks(
        proposition_claim,
        prop_cluster_dict,
        max_sampled_chunks_per_cluster,
        n_chunks_needed_per_cluster,
    ...
    if prop_informative_chunks[0] == []:
        return "ERROR"
    prop_final_args = get_final_args(
        proposition_claim,
        prop_cluster_dict,
        max_sampled_chunks_per_cluster,
        prop_informative_chunks,
    )
    master_dict.update(
        {
            "prop_final_args": prop_final_args,
    ...
    opp_informative_chunks = get_n_informative_chunks(
        opposition_claim,
        opp_cluster_dict,
        max_sampled_chunks_per_cluster,
        n_chunks_needed_per_cluster,
    ...
    if opp_informative_chunks[0] == []:
        return master_dict
    opp_final_args = get_final_args(
        opposition_claim,
        opp_cluster_dict,
        max_sampled_chunks_per_cluster,
        opp_informative_chunks,
    )
    master_dict.update(
        {
            "opp_final_args": opp_final_args,
    ...
    # Get final judgment
    final_judge = get_final_judgement(
        arg1_w_claims, arg2_w_claims, use_small_model=use_small_model
    )
    idx = int(final_judge["argument"]) - 1
    choice = [master_dict["proposition_claim"], master_dict["opposition_claim"]][idx]
    master_dict["victor"] = choice
